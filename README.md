...
- Agent auth, cookie vault, and MCP SSRF review: [USD $299 focused review for token brokers, cookie vaults, site_login, OAuth/HITL consent, authenticated scraping, dynamic URL fetch/SSRF-with-credentials, and cache isolation](https://jackjin1997.github.io/agent-audit-sprint/agent-auth-security-review.html); [open the focused intake](https://github.com/jackjin1997/agent-audit-sprint/issues/new?template=agent-auth-review.yml) to request scope before any payment.
...
- GitHub Code Scanning/SARIF: [copy the workflow](https://jackjin1997.github.io/agent-audit-sprint/mcp-code-scanning-github-action.html) or use `jackjin1997/agent-mcp-code-scan-action@v1` with `sarif: "true"`; current `@v1` also flags dynamic URL fetch and SSRF-with-credentials evidence.
